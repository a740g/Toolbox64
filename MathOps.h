...
#define GetRandomMaximum() RAND_MAX
...
inline constexpr qb_bool IsLongEven(int32_t n)
{
...
/// @param n Any integer
/// @return True if n is even
inline constexpr qb_bool IsInteger64Even(int64_t n)
{
    return TO_QB_BOOL((n & 1) == 0);
}
...
inline constexpr qb_bool IsLongPowerOf2(uint32_t n)
{
...
/// @param n A number
/// @return True if n is a power of 2
inline constexpr qb_bool IsInteger64PowerOf2(uint64_t n)
{
    return TO_QB_BOOL(n && !(n & (n - 1)));
}
...
inline constexpr uint32_t RoundLongUpToPowerOf2(uint32_t n)
{
    --n;
    n |= n >> 1;
    n |= n >> 2;
...
/// @param n Any number
/// @return Next (ceiling) power of 2 for x
inline constexpr uint64_t RoundInteger64UpToPowerOf2(uint64_t n)
{
    --n;
    n |= n >> 1;
    n |= n >> 2;
...
inline constexpr uint32_t RoundLongDownToPowerOf2(uint32_t n)
{
    n |= (n >> 1);
    n |= (n >> 2);
    n |= (n >> 4);
...
/// @param n Any number
/// @return Previous (floor) power of 2 for x
inline constexpr uint64_t RoundInteger64DownToPowerOf2(uint64_t n)
{
    n |= n >> 1;
    n |= n >> 2;
    n |= n >> 4;
...
inline constexpr int32_t GetDigitFromLong(uint32_t n, uint32_t p)
{
    switch (p)
    {
    case 0:
...
inline constexpr int32_t AverageLong(int32_t x, int32_t y)
{
    return (x & y) + ((x ^ y) / 2);
}
...
/// @param y A number
/// @return Average of x & y
inline constexpr int64_t AverageInteger64(int64_t x, int64_t y)
{
    return (x & y) + ((x ^ y) / 2);
}
...
inline constexpr int32_t RemapLong(int32_t value, int32_t oldMin, int32_t oldMax, int32_t newMin, int32_t newMax)
{
...
/// @param newMax New range maximum
/// @return The value remapped to the new range
inline constexpr int64_t RemapInteger64(int64_t value, int64_t oldMin, int64_t oldMax, int64_t newMin, int64_t newMax)
{
    return (value - oldMin) * (newMax - newMin) / (oldMax - oldMin) + newMin;
}
...
/// @param newMax New range maximum
/// @return The value remapped to the new range
inline constexpr float RemapSingle(float value, float oldMin, float oldMax, float newMin, float newMax)
{
    return (value - oldMin) * (newMax - newMin) / (oldMax - oldMin) + newMin;
}
...
/// @param newMax New range maximum
/// @return The value remapped to the new range
inline constexpr double RemapDouble(double value, double oldMin, double oldMax, double newMin, double newMax)
{
    return (value - oldMin) * (newMax - newMin) / (oldMax - oldMin) + newMin;
}
...
inline constexpr float LerpSingle(float start, float end, float amount)
{
...
/// @param amount An amount (usually 0.0 - 1.0)
/// @return An interpolated value
inline constexpr double LerpDouble(double start, double end, double amount)
{
    return start + amount * (end - start);
}
...
inline constexpr float NormalizeSingle(float value, float start, float end)
{
...
/// @param end The ending value of the input range
/// @return A normalized value between 0.0 and 1.0
inline constexpr double NormalizeDouble(double value, double start, double end)
{
    return (value - start) / (end - start);
}
