...
// Single-file implementation, based on software revision: 8d048d9
//
// =============================================================================
...
#define RET_FAIL(x, e)                                     \
    ...
    {                                                      \
        ...
        fmidi__err.file = __FILE__;                        \
        fmidi__err.line = __LINE__;                        \
        fmidi__err.code = (e);                             \
        return (x);                                        \
    } while (0)
#else
...
void WriterT<T>::writeLE(const void *data, size_t size)
{
    ...
    {
    case 0x11:
        static_cast<T *>(this)->rwrite(data, size);
        break;
    case 0x44:
        static_cast<T *>(this)->write(data, size);
        break;
    default:
        assert(false);
    }
...
const char *fmidi_strerror(fmidi_status_t status)
{
    ...
    case fmidi_err_input:
        return "input error";
    case fmidi_err_largefile:
        return "file too large";
    case fmidi_err_output:
        return "output error";
    }
    return nullptr;
}
...
void fmidi_player_goto_time(fmidi_player_t *plr, double time)
{
         ...
         fmidi_seq_peek_event(&seq, &sqevt) && sqevt.time < time;)
    {
        ...
        if (evt.type == fmidi_event_message)
        {
            ...
            if (status >> 4 == 0b1100 && evt.datalen == 2)
            { // program change
                uint8_t channel = status & 0xf;
                programs[channel] = evt.data[1] & 127;
            }
            else if (status >> 4 == 0b1011 && evt.datalen == 3)
            { // control change
                uint8_t channel = status & 0xf;
...
enum memstream_status
{ // make it match fmidi status codes
    ms_ok,
    ms_err_format,
    ms_err_eof,
};
...
fmidi_smf_t *fmidi_mus_mem_read(const uint8_t *data, size_t length)
{
    ...
    for (bool score_end = false; !score_end;)
    {
        ...
        case 2:
        {
            ...
            if ((ms = mb.readintLE(&data1, 1)))
                RET_FAIL(nullptr, fmidi_err_format);
            uint32_t bend = (data1 < 128) ? (data1 << 6) : (8192 + (data1 - 128) * 8191 / 127);
