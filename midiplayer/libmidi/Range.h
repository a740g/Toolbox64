...
{
public:
    range_t() noexcept { Clear(); }

    range_t(const range_t &) = delete;
    range_t(range_t &&other) noexcept { Set(other._Begin, other._End); }
    ...
    void Clear() noexcept { Set(std::numeric_limits<uint32_t>::max(), std::numeric_limits<uint32_t>::max()); }

    uint32_t Size() const noexcept { return _End - _Begin; }

    bool HasBegin() const noexcept { return _Begin != std::numeric_limits<uint32_t>::max(); }
    bool HasEnd() const noexcept { return _End != std::numeric_limits<uint32_t>::max(); }
