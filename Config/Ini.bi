...
SUB Ini_SortSection (file$, __section$)
    ...
    DO
        tempValue$ = Ini_ReadSetting(file$, __section$, _STR_EMPTY)
    ...
    IF NOT Sort_StringArray(keys()) THEN __ini.code = 23: EXIT SUB
...
SUB Ini_DeleteKey (file$, __section$, __key$)
    ...
    key$ = _TRIM$(__key$)
    IF LEN(key$) = 0 THEN __ini.code = 12: EXIT SUB
    __ini.lastKey = key$
...
SUB __Ini_Commit
    ...
    IF LEN(__ini.currentFileName) = 0 THEN __ini.code = 18: EXIT SUB
...
FUNCTION __Ini_GetSectionData$ (__section$)
    ...
    IF LEN(__ini.currentFileName) = 0 THEN __ini.code = 18: EXIT FUNCTION
...
FUNCTION Ini_ReadSetting$ (file$, __section$, __key$)
    ...
    IF __ini.position > 0 THEN
        ...
        IF LEN(tempValue$) > 0 THEN
            IF LEFT$(tempValue$, 1) = _CHR_QUOTE THEN
                tempValue$ = MID$(tempValue$, 2)
                Quote = INSTR(tempValue$, _CHR_QUOTE)
                IF Quote > 0 THEN
...
SUB Ini_WriteSetting (file$, __section$, __key$, __value$)
    ...
    IF LTRIM$(STR$(VAL(value$))) <> value$ THEN
        ...
        IF INSTR(value$, _CHR_SPACE) THEN value$ = _CHR_QUOTE + value$ + _CHR_QUOTE
    END IF
    ...
    IF __ini.code = 1 OR __ini.code = 17 THEN
        ...
        IF LEN(file$) = 0 THEN file$ = __ini.currentFileName
        IF LEN(file$) = 0 THEN __ini.code = 21: EXIT SUB
...
SUB Ini_Close
    ...
    IF LEN(__ini.currentFileName) = 0 THEN EXIT SUB
    ...
    __ini.currentFileName = _STR_EMPTY
END SUB
...
SUB Ini_Load (file$)
    ...
    IF LEN(file$) _ANDALSO __ini.currentFileName <> file$ THEN __ini.currentFileName = _STR_EMPTY
    ...
    IF LEN(__ini.currentFileName) = 0 THEN
        'initialization
        IF _FILEEXISTS(file$) THEN
            ...
            IF INSTR(__ini.wholeFile, _CHR_CR) THEN __ini.LF = _STR_CRLF ELSE __ini.LF = _CHR_LF
        ELSE
            IniFileNotFound:
            __ini.code = 1
...
FUNCTION Ini_GetNextKey$
    ...
    IF __ini.sectionData <> lastDataBlock$ THEN
        ...
        IF RIGHT$(__ini.sectionData, LEN(__ini.LF)) <> __ini.LF THEN tempLF$ = __ini.LF ELSE tempLF$ = _STR_EMPTY
    END IF
    ...
    IF INSTR(tempKey$, _CHR_LF) > 0 THEN
        position = position + INSTR(tempKey$, _CHR_LF) + 1
        tempKey$ = MID$(tempKey$, INSTR(tempKey$, _CHR_LF) + 1)
    END IF
...
FUNCTION Ini_GetInfo$
    ...
    SELECT CASE __ini.code
        CASE INI_INFO_SUCCESS: Ini_GetInfo = "Operation successful"
        CASE INI_INFO_FILE_NOT_FOUND: Ini_GetInfo = "ERROR: File not found"
        CASE INI_INFO_EMPTY_VALUE: Ini_GetInfo = "Empty value"
        CASE INI_INFO_KEY_NOT_FOUND: Ini_GetInfo = "ERROR: Key not found"
        CASE INI_INFO_KEY_UPDATED: Ini_GetInfo = "Key updated"
        CASE INI_INFO_GLOBAL_KEY_CREATED: Ini_GetInfo = "Global key created"
        CASE INI_INFO_KEY_CREATED_EXISTING_SECTION: Ini_GetInfo = "Key created in existing section"
        CASE INI_INFO_NO_CHANGE_SAME_VALUE: Ini_GetInfo = "No changes applied (same value)"
        CASE INI_INFO_NEW_SECTION_KEY_CREATED: Ini_GetInfo = "New section created; key created"
        CASE INI_INFO_NO_MORE_KEYS: Ini_GetInfo = "No more keys"
        CASE INI_INFO_FILE_CREATED_KEY_ADDED: Ini_GetInfo = "File created; new key added"
        CASE INI_INFO_INVALID_KEY: Ini_GetInfo = "ERROR: Invalid key"
        CASE INI_INFO_SECTION_DELETED: Ini_GetInfo = "Section deleted"
        CASE INI_INFO_SECTION_NOT_FOUND: Ini_GetInfo = "ERROR: Section not found"
        CASE INI_INFO_INVALID_SECTION: Ini_GetInfo = "ERROR: Invalid section"
        CASE INI_INFO_NEW_SECTION_KEY_MOVED: Ini_GetInfo = "New section created; existing key moved"
        CASE INI_INFO_EMPTY_FILE: Ini_GetInfo = "ERROR: Empty file"
        CASE INI_INFO_NO_FILE_OPEN: Ini_GetInfo = "ERROR: No file open"
        CASE INI_INFO_KEY_DELETED: Ini_GetInfo = "Key deleted"
        CASE INI_INFO_KEY_MOVED: Ini_GetInfo = "Key moved"
        CASE INI_INFO_INVALID_FILENAME: Ini_GetInfo = "ERROR: Invalid file name/path"
        CASE INI_INFO_SECTION_SORTED: Ini_GetInfo = "Section sorted"
        CASE INI_INFO_SECTION_ALREADY_SORTED: Ini_GetInfo = "No changes applied; section already sorted"
        CASE INI_INFO_NO_MORE_SECTIONS: Ini_GetInfo = "No more sections"
...
FUNCTION Ini_GetCode&
    SHARED __ini AS __IniType
    Ini_GetCode = __ini.code
END FUNCTION

FUNCTION Ini_GetLastSection$
    SHARED __ini AS __IniType
    Ini_GetLastSection = __ini.lastSection
END FUNCTION

FUNCTION Ini_GetLastKey$
    SHARED __ini AS __IniType
    Ini_GetLastKey = __ini.lastKey
END FUNCTION
