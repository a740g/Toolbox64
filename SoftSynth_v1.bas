            ...
            IF __Voice(v).snd >= 0 THEN
                ...
                IF LEN(__SampleData(__Voice(v).snd)) >= SIZE_OF_SINGLE THEN
                    ...
                    DO WHILE s < frames
                        ...
                        IF SOFTSYNTH_VOICE_PLAY_FORWARD_LOOP = __Voice(v).mode AND __Voice(v).position > __Voice(v).endPosition THEN
                            ' Reset loop position if we reached the end of the loop
                            __Voice(v).position = __Voice(v).startPosition
                        ELSEIF __Voice(v).position > __Voice(v).endPosition THEN
                        ...
                        outputFrame = PeekStringSingle(__SampleData(__Voice(v).snd), __Voice(v).position) * __Voice(v).volume ' just calculate this once
                        ...
                        __SoftSynth_SoundBuffer(i) = __SoftSynth_SoundBuffer(i) + outputFrame * (0.5! - __Voice(v).balance)
                        i = i + 1
                        __SoftSynth_SoundBuffer(i) = __SoftSynth_SoundBuffer(i) + outputFrame * (0.5! + __Voice(v).balance)
                        i = i + 1
