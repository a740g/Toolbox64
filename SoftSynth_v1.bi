...
$IF SOFTSYNTH_BI = UNDEFINED THEN
    ...
    TYPE __VoiceType
        ...
        pitch AS DOUBLE ' the mixer uses this to step through the sample correctly
        frequency AS _UNSIGNED LONG ' the voice frequency (this is used to calculate the pitch)
        position AS DOUBLE ' sample frame position in the sample buffer (updated by pitch)
        ...
        oldFrame AS SINGLE ' this is the last frame played (used for linear interpolation)
    END TYPE
